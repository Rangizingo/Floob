"""
Floob 2.0 Transparent Overlay Window.

Updated window class integrated with the new animation engine.
Handles window creation, positioning, dragging, click events,
and smooth animation at 30 FPS.
"""
...
class PetWindow:
    ...
    Integrated with the animation engine for smooth 30 FPS rendering.
    """
    ...
    # Frame rate
    TARGET_FPS = 30
    FRAME_TIME_MS = int(1000 / TARGET_FPS)  # ~33ms per frame

    def __init__(self) -> None:
        """Initialize the pet window."""
        self.root = tk.Tk()
        self.root.title("Floob 2.0")
        ...
        # Animation callback
        self._on_frame_update: Optional[Callable[[float], None]] = None
    ...
    def _on_button_release(self, event: tk.Event) -> None:
        ...
        if not self._is_dragging and self._on_left_click:
            self._on_left_click()
        elif self._is_dragging and self._on_drag_end:
            self._on_drag_end()
        self._is_dragging = False
    ...
    def set_on_drag_start(self, callback: Callable[[], None]) -> None:
        """Set callback for when dragging starts."""
        self._on_drag_start = callback

    def set_on_drag_end(self, callback: Callable[[], None]) -> None:
        """Set callback for when dragging ends."""
        self._on_drag_end = callback

    def set_on_frame_update(self, callback: Callable[[float], None]) -> None:
        """
        Set callback for animation frame updates.

        The callback receives delta_time in seconds since last frame.
        """
    ...
    def get_center(self) -> tuple[int, int]:
        ...
        Returns:
            Tuple of (center_x, center_y) in window coordinates.
        """
    ...
    def start_animation_loop(self) -> None:
        """Start the animation loop at TARGET_FPS."""
        self._last_frame_time = time.time()
    ...
    def stop_animation_loop(self) -> None:
        ...
        if self._animation_timer:
            self.cancel_schedule(self._animation_timer)
    ...
    def _animation_loop(self) -> None:
        ...
        if self._on_frame_update:
            self._on_frame_update(delta_time)

        # Schedule next frame
        self._animation_timer = self.root.after(self.FRAME_TIME_MS, self._animation_loop)
    ...
    def update(self) -> None:
        """Process pending events (for testing)."""
        self.root.update()

    def update_idletasks(self) -> None:
        """Process pending idle tasks (for testing)."""
